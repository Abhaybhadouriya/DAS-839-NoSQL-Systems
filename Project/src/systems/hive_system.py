    ...
    def __init__(self):
        ...
        with self.conn.cursor() as cursor:
            cursor.execute(f"""
                ...
                PARTITIONED BY (sid STRING, course STRING)
                ROW FORMAT DELIMITED
                FIELDS TERMINATED BY '\t'
                STORED AS TEXTFILE
            """)
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                if not cursor.fetchone():
                    ...
                    cursor.execute(f"""
                        ALTER TABLE {self.table} ADD IF NOT EXISTS
                        PARTITION (sid='{sid}', course='{course}')
                    """)
                    # Load data directly
                    cursor.execute(f"""
                        LOAD DATA LOCAL INPATH '{temp_file}'
        ...
        with self.conn.cursor() as cursor:
            try:
                ...
                if partition_exists:
                    ...
                    """)
                    self.conn.commit()
                    print(f"Successfully deleted partition: {sid}/{course}")
                else:
                    print(f"Partition not found: {sid}/{course}")
                ...
                if op["operation"] in ["SET"] and op["grade"]:
                    ...
                    try:
                        with self.conn.cursor() as cursor:
                            ...
                            cursor.execute(f"""
                                SHOW PARTITIONS {self.table} 
                                PARTITION(sid='{op["sid"]}', course='{op["course"]}')
                            """)
                            partition_exists = cursor.fetchone() is not None
                            ...
                            if partition_exists:
                                ...
                                """)
                                self.conn.commit()
                                print(f"Successfully deleted partition: {op['sid']}/{op['course']}")
                            else:
                                print(f"Partition not found: {op['sid']}/{op['course']}")
