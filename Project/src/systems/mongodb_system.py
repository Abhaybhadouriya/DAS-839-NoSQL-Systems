...
class MongoDBSystem:
    ...
    def read(self, sid, course):
        result = self.collection.find_one({"sid": sid, "course": course})
    ...
    def merge(self, other_system_name):
        ...
        for op in operations:
            ...
            if op["operation"] in ["SET"] and op["grade"]:
                ...
                existing = self.collection.find_one({"sid": op["sid"], "course": op["course"]})
                if existing and existing.get("timestamp", "1970-01-01T00:00:00") < op["timestamp"]:
                    self.collection.delete_one({"sid": op["sid"], "course": op["course"]})
                    # self.oplog.log_operation(self.op_id, "DELETE", (op["sid"], op["course"]), timestamp=datetime.now().isoformat())
                    self.oplog.log_operation(self.op_id, "DELETE", (op["sid"], op["course"]), timestamp=op["timestamp"])
